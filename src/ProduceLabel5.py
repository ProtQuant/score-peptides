# 4/7/2021 occurrence of pep in port should obey the splitting rule
import numpy
import pandas as pd
import time
    ...
    for filepath in filepaths:
        ...
        for i in range(len(df)):
            if numpy.isnan(df['intensity'][i]):
                continue
            pep = df['peptide'][i]
            count = df['intensity'][i]
            if (pep not in pepDict):
                pepDict[pep] = {'intensity': 0, 'occurrence': 0, 'proteinIndex': []}  # will be updated later
                pepDict[pep]['intensity'] = count
                pepList.append(pep)
            elif (pepDict[pep]['intensity'] < count):
                pepDict[pep]['intensity'] = count
...
def main():
    ...
    #
    # protList, protDict_temp = init_proteins(fastaFile)
    ...
    # protDict_temp, pepDict = update_occurrence(allPossiblePepDict, protList, protDict_temp, pepList, pepDict)
    # t3 = time.perf_counter()
    # print('finish updating occurrence....\t' + str(t3 - t2))  # 99% of total time
    # pepDict = update_all_unseen_peptides(protList, protDict_temp, pepList, pepDict)
    ...
    csvFiles = ['.\\Testcases2\\Test1.csv']
    pepList, pepDict = init_peptides(csvFiles)
    allPossiblePepDict = find_all_possible_simple_peptides(protList)
    ...
    pepDf = pd.DataFrame({'peptide': pepDict.keys(),
                          ...
                          'protIndex': [pepDict[k]['proteinIndex'] for k in pepDict],
                          'occurrence': [pepDict[k]['occurrence'] for k in pepDict]})
    pepDf.to_csv('.\\Testcases2\\5_Test1_pepDf.csv', index=True, sep=',')

    protDf = pd.DataFrame({"protein": protDict.keys(), 'pepIndex': [protDict[k]['seenPeptides'] for k in protDict]})
    protDf['count'] = protDf.apply(lambda x: len(x['pepIndex']), axis=1)
    protDf.to_csv('.\\Testcases2\\5_Test1_protDf.csv', index=True, sep=',')
